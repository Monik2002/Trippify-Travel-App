...
// New function to get all journals without filtering by createdBy
const getAllJournalsAdmin = async (req, res) => {
  const journals = await Journal.find().populate('createdBy', 'username email');
  res.status(StatusCodes.OK).json({ journals });
};
